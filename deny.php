...
if (isset($_POST['ip_address']) && !empty($_POST['ip_address'])) {
  ...
  if (strpos($htaccess_contents, "Require not ip $ip_address") === false) {
    ...
    if (!empty($hostname)) {$htaccess_contents = str_replace('</RequireAll>', "Require not ip $ip_address\nRequire not host $hostname\n</RequireAll>", $htaccess_contents);} 
    else {$htaccess_contents = str_replace('</RequireAll>', "Require not ip $ip_address\n</RequireAll>", $htaccess_contents);}
    ...
    // Close .htaccess file
    fclose($htaccess_file);echo "IP address $ip_address and hostname $hostname have been blocked.";} else {echo "IP address $ip_address is already blocked.";}}
?>
